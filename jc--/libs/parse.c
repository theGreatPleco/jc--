...
byte heading[] = {brack_open, brack_close, para_open, para_close, end_of_statement};
...
{
  return bytescmp(buffer, "_main") ? t_main : t_decl_func;
}
...
static token_t check_is_type(byte *buffer)
{
  if(!buffer) exit(-1);
  token_t type = t_none;
  if(bytescmp(buffer, "int"))
  {
    type = t_decl_int;
  }
  if(bytescmp(buffer, "byte"))
  {
    type = t_decl_byte;
  }
  return type; 
...
static void lex_first_pass(void)
{
  token_t type = t_none;
  int32 stackpos = 0;
  for(leaf = head; leaf != 0;)
  {
    ...
    if(*buffer == method_decl) type = is_main_or_decl(buffer);
    if(*buffer == method_call) type = t_call_func;

    if(*buffer == '\"') type = t_bytes;
    if(*buffer == '=') type = t_asign;
    ...
    printf("prev is %s and end is %s\n", end ? end->val : "prev is null", buffer);
